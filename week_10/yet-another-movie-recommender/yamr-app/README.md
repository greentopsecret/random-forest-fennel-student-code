...
```bash
yamr-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
yamr-app$ sam local start-api
yamr-app$ curl http://localhost:3000/
```
...
```bash
yamr-app$ sam logs -n HelloWorldFunction --stack-name yamr-app --tail
```
...
```bash
yamr-app$ pip install pytest pytest-mock --user
yamr-app$ python -m pytest tests/ -v
```
